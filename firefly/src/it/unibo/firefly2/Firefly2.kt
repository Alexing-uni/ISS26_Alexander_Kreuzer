...
class Firefly2 ( name: String, scope: CoroutineScope, isconfined: Boolean=false, isdynamic: Boolean=false ) : 
	...
	override fun getBody() : (ActorBasicFsm.() -> Unit){
		...
		return { //this:ActionBasciFsm
				state("s0") { //this:State
					action { //it:State
						CommUtils.outcyan("$name | fase 300ms")
						delay(300) 
				...
				state("charging") { //this:State
					...
					sysaction { //it:State
					 ...
					 transition(edgeName="t02",targetState="doFlash",cond=whenTimeout("local_tout_"+name+"_charging"))   
					transition(edgeName="t03",targetState="perceiveFlash",cond=whenEvent("flash"))
				}	 
				...
				state("doFlash") { //this:State
					action { //it:State
						CommUtils.outcyan("$name | *** FLASH! ***")
						emit("flash", "flash(firefly2)" ) 
